...
import initCppJs from 'gdal3.js/browser/st/js';
...
export default {
    ...
    mounted() {
        initCppJs({path: 'dist', useWorker: false}).then((gdalInstance) => {
            Module = gdalInstance;
    ...
    methods: {
        translate() {
            ...
            this.datasets.forEach(d => {
                ...
                promises.push(d[this.gdalFunction](`/memfs/${fileName}.${ext}`, Module.toVector(Module.VectorString, options)));
            });
            Promise.allSettled(promises).then((results) => {
                results.forEach(({value: d}) => d.close());
                const files = Module.getFileList('/memfs');

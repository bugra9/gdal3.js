...
#include <gdal3js/Driver.h>
#include <gdal3js/GCP.h>
...
Dataset::~Dataset() { CPLFree(this->ptr); }
int Dataset::buildOverviews(std::string resampling,
  ...
  return (int)GDALBuildOverviewsEx(
      (GDALDatasetH)this->ptr, resampling.c_str(), overviewlist.size(),
      overviewlist.data(), 0, 0, NULL, NULL,
      options.size() == 0 ? NULL : options2.data());
}
...
int Dataset::getRasterXSize() { return GDALGetRasterXSize(this->ptr); }
int Dataset::getRasterYSize() { return GDALGetRasterYSize(this->ptr); }
int Dataset::getRasterCount() { return GDALGetRasterCount(this->ptr); }
int Dataset::close() { return GDALClose(this->ptr); }
...
int Dataset::getGCPCount() { return GDALGetGCPCount(this->ptr); }
std::string Dataset::getGCPProjection() {
  return Gdal::charPtrToString(GDALGetGCPProjection(this->ptr));
}
int Dataset::setGCPs(std::vector<std::shared_ptr<GCP>> nGCPs,
                     std::string pszGCPProjection) {
  int arg2 = nGCPs.size();
  GDAL_GCP *arg3 = (GDAL_GCP *)0;
  ...
  return GDALSetGCPs(this->ptr, nGCPs.size(), (GDAL_GCP const *)arg3,
                     pszGCPProjection.c_str());
}
int Dataset::flushCache() { return (int)GDALFlushCache(this->ptr); }
int Dataset::addBand(int datatype, std::vector<std::string> options) {
  ...
  return GDALAddBand(this->ptr, (GDALDataType)datatype,
                     options.size() == 0 ? NULL : options2.data());
}
...
int Dataset::getLayerCount() { return GDALDatasetGetLayerCount(this->ptr); }
bool Dataset::isLayerPrivate(int index) {
  return GDALDatasetIsLayerPrivate(this->ptr, index);
}
void Dataset::resetReading() { GDALDatasetResetReading(this->ptr); }
bool Dataset::testCapability(std::string cap) {
  return (GDALDatasetTestCapability(this->ptr, cap.c_str()) > 0);
}
int Dataset::abortSQL() { return GDALDatasetAbortSQL(this->ptr); }
...
void Dataset::clearStatistics() { GDALDatasetClearStatistics(this->ptr); }
std::vector<std::string>
Dataset::getFieldDomainNames(std::vector<std::string> options) {
  std::vector<const char *> options2;
  for (auto &a : options) {
    options2.push_back(a.c_str());
  }
  ...
  return Gdal::charPtrPtrToStringVector(GDALDatasetGetFieldDomainNames(
...
std::vector<std::string>
Dataset::getRelationshipNames(std::vector<std::string> options) {
  std::vector<const char *> options2;
  for (auto &a : options) {
    options2.push_back(a.c_str());
  }
  ...
  return Gdal::charPtrPtrToStringVector(GDALDatasetGetRelationshipNames(
      this->ptr, options.size() == 0 ? NULL : options2.data()));
}
...
std::string Dataset::info(std::vector<std::string> psOptions) {
  ...
  GDALInfoOptions *options = GDALInfoOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
...
std::string Dataset::vectorInfo(std::vector<std::string> psOptions) {
  ...
  GDALVectorInfoOptions *options = GDALVectorInfoOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
...
std::string Dataset::multiDimInfo(std::vector<std::string> psOptions) {
  ...
  GDALMultiDimInfoOptions *options = GDALMultiDimInfoOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
...
Dataset::translate(std::string pszDestFilename,
  ...
  GDALTranslateOptions *options = GDALTranslateOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result =
      GDALTranslate(pszDestFilename.c_str(), this->ptr, options, &usageError);
...
std::shared_ptr<Dataset>
Dataset::vectorTranslate(std::string pszDest,
                         std::vector<std::string> psOptions) {
  ...
  GDALVectorTranslateOptions *options = GDALVectorTranslateOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result = GDALVectorTranslate(pszDest.c_str(), 0, pahSrcDS.size(),
                                    pahSrcDS.data(), options, &usageError);
...
std::shared_ptr<Dataset>
Dataset::vectorTranslate2(std::shared_ptr<Dataset> hDstDS,
                          std::vector<std::string> psOptions) {
  std::vector<void *> pahSrcDS;
  pahSrcDS.push_back(this->ptr);
  ...
  GDALVectorTranslateOptions *options = GDALVectorTranslateOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result = GDALVectorTranslate(0, hDstDS->ptr, pahSrcDS.size(),
                                    pahSrcDS.data(), options, &usageError);
  GDALVectorTranslateOptionsFree(options);
  if (result == 0)
    return NULL;
...
std::shared_ptr<Dataset>
Dataset::demProcessing(std::string pszDestFilename, std::string pszProcessing,
                       std::string pszColorFilename,
                       std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
  ...
  GDALDEMProcessingOptions *options = GDALDEMProcessingOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result = GDALDEMProcessing(
      pszDestFilename.c_str(), this->ptr, pszProcessing.c_str(),
      pszColorFilename.c_str(), options, &usageError);
...
std::shared_ptr<Dataset>
Dataset::nearblack(std::string pszDest, std::shared_ptr<Dataset> hDstDS,
                   std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
  ...
  GDALNearblackOptions *options = GDALNearblackOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result = GDALNearblack(pszDest.c_str(), hDstDS->ptr, this->ptr, options,
                              &usageError);
...
std::shared_ptr<Dataset> Dataset::grid(std::string pszDest,
                                       std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
  ...
  GDALGridOptions *options = GDALGridOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
...
std::shared_ptr<Dataset>
Dataset::rasterize(std::string pszDest, std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
  }
  ...
  int usageError;
  auto result =
      GDALRasterize(pszDest.c_str(), 0, this->ptr, options, &usageError);
...
std::shared_ptr<Dataset>
Dataset::rasterize(std::shared_ptr<Dataset> hDstDS,
                   std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
...
std::shared_ptr<Dataset>
Dataset::footprint(std::string pszDest, std::shared_ptr<Dataset> hDstDS,
                   std::vector<std::string> psOptions) {
  std::vector<const char *> psOptions2;
  for (auto &a : psOptions) {
    psOptions2.push_back(a.c_str());
  ...
  GDALFootprintOptions *options = GDALFootprintOptionsNew(
      psOptions.size() == 0 ? NULL : ((char **)psOptions2.data()), NULL);
  int usageError;
  auto result = GDALFootprint(pszDest.c_str(), hDstDS->ptr, this->ptr, options,
                              &usageError);
...
// =================================================== DATASET - PRIVATE
// ========
Dataset::Dataset(void *dataset) { this->ptr = dataset; }

struct Dataset::MakeSharedEnabler : public Dataset {
  template <typename... Args>
  MakeSharedEnabler(Args &&...args) : Dataset(std::forward<Args>(args)...) {}
};

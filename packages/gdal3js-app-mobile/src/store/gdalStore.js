...
import { initCppJs, AllSymbols, Gdal } from 'gdal3.js/Gdal.h';
...
class GdalStore {
    ...
    constructor() {
        ...
        console.log('oooleyy2');
        initCppJs().then(() => {
            console.log('oooleyy');
            Gdal.allRegister();
            this.drivers = AllSymbols.toArray(Gdal.getDrivers()).sort(
    ...
    open(file) {
        console.log('---open----', AllSymbols);
        const dataset = Gdal.openEx(file.replace('file://', ''));
        console.log('---open----2', dataset);
        if (dataset) {
            let info = JSON.parse(
                dataset.info(AllSymbols.toVector(AllSymbols.VectorString, ['-json'])),
            );
            const hasSize =
                info.size &&
                info.size.length >= 2 &&
            ...
            if (type === 'vector') {
                info = JSON.parse(
                    dataset.vectorInfo(
                        AllSymbols.toVector(AllSymbols.VectorString, ['-json']),
                    ),
                );
            }
            ...
            this.datasets.forEach((d, dIndex) => {
                ...
                promises.push(
                    d[this.program](
                        ...
                        AllSymbols.toVector(AllSymbols.VectorString, options),
                    ),
                );
            });
...
initCppJs().then(() => {
    Gdal.allRegister();
    drivers = AllSymbols.toArray(Gdal.getDrivers()).sort((a, b) => a.getShortName().localeCompare(b.getShortName()));
    console.log('basla3');
...
export function open(file) {
    const dataset = Gdal.openEx(file.replace('file://', ''));
    if (dataset) {
        let info = JSON.parse(dataset.info(AllSymbols.toVector(AllSymbols.VectorString, ['-json'])));
        const hasSize = info.size && info.size.length >= 2 && (info.size[0] > 0 || info.size[1] > 0);
        const type = info.bands.length > 0 && hasSize ? 'raster' : 'vector';
        if (type === 'vector') {
            info = JSON.parse(dataset.vectorInfo(AllSymbols.toVector(AllSymbols.VectorString, ['-json'])));
        }
...
async function vector() {
    ...
    const filePath = await RNFetchBlob.config({fileCache: true})
        ...
        .then(res => res.path());
    console.log('data', filePath, Gdal.openEx);

    const dataset = Gdal.openEx(filePath);
    console.log(dataset);
    if (dataset) {
        ...
        const options = new AllSymbols.VectorString();
        options.push_back('-f');

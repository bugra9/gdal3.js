...
async function createTest() {
    ...
    describe('ZSTD-compressed GeoTIFF', () => {
        it('decodes TIFF with ZSTD compression without missing-codec error', async () => {
            const name = 'testGeoTiff_withZstd';
            let file = `data/${name}.tif`;
            if (!isNode) {
                const fileData = await fetch(file);
                file = new File([await fileData.blob()], `${name}.tif`);
            ...
            // Force a full pixel decode via translate. Without the ZSTD codec
            // linked into libtiff, this path emits "Cannot open TIFF file due
            // to missing codec ZSTD" and the resulting file is invalid.
            const out = await Gdal.gdal_translate(dataset, ['-of', 'GTiff', '-co', 'COMPRESS=NONE']);
            const result2 = await Gdal.open(out.real);
            const dataset2 = result2.datasets[0];
            assert.strictEqual(dataset2.pointer > 0, true, 'decoded output invalid (ptr == 0)');
    ...
    describe('LERC-compressed GeoTIFF', () => {
        it('decodes TIFF with LERC compression without missing-codec error', async () => {
            const name = 'testGeoTiff_withLerc';
            let file = `data/${name}.tif`;
            if (!isNode) {
                const fileData = await fetch(file);
                file = new File([await fileData.blob()], `${name}.tif`);
            ...
            // Force a full pixel decode via translate. Without the LERC codec
            // linked into libtiff, this path emits "Cannot open TIFF file due
            // to missing codec LERC" and the resulting file is invalid.
            const out = await Gdal.gdal_translate(dataset, ['-of', 'GTiff', '-co', 'COMPRESS=NONE']);
            const result2 = await Gdal.open(out.real);
            const dataset2 = result2.datasets[0];
            assert.strictEqual(dataset2.pointer > 0, true, 'decoded output invalid (ptr == 0)');
    ...
    describe('LERC_ZSTD-compressed GeoTIFF', () => {
        it('decodes TIFF with LERC + ZSTD secondary deflate combination', async () => {
            const name = 'testGeoTiff_withLercZstd';
            let file = `data/${name}.tif`;
            if (!isNode) {
                const fileData = await fetch(file);
                file = new File([await fileData.blob()], `${name}.tif`);
            ...
            // LERC_ZSTD applies a ZSTD secondary deflate on top of LERC primary
            // compression. Both codecs must be linked into libtiff for this
            // path to succeed.
            const out = await Gdal.gdal_translate(dataset, ['-of', 'GTiff', '-co', 'COMPRESS=NONE']);
            const result2 = await Gdal.open(out.real);
            const dataset2 = result2.datasets[0];

...
# ZSTD #
###########
ZSTD_SRC = $(SRC_DIR)/zstd-$(ZSTD_VERSION)

zstd: $(ROOT_DIR)/lib/libzstd.a

$(ROOT_DIR)/lib/libzstd.a: $(ZSTD_SRC)/build/cmake/_build/Makefile
	cd $(ZSTD_SRC)/build/cmake/_build; \
	$(EMMAKE) make -j4 install;

$(ZSTD_SRC)/build/cmake/_build/Makefile: $(ZSTD_SRC)/build/cmake/CMakeLists.txt
...
$(ZSTD_SRC)/build/cmake/CMakeLists.txt:
	mkdir -p $(SRC_DIR); \
	cd $(SRC_DIR); \
	wget -nc $(ZSTD_URL); \
